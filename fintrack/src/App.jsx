...
function App() {
  ...
  const totalIncome = data.transactions.filter(t => t.type === "income" && !t.isGoalAccountTx).reduce((s, t) => s + Number(t.amount || 0), 0);
  const totalExpense = data.transactions.filter(t => t.type === "expense" && !t.isGoalAccountTx).reduce((s, t) => s + Number(t.amount || 0), 0);
  // Net worth = sum of all bank balances (linked transactions) + unlinked transactions
  // Goal account transactions (isGoalAccountTx=true) are excluded from all net worth maths
  const linkedBankIds = new Set((data.banks || []).map(b => String(b.id)));
  const goalAccountIds = new Set((data.goalAccounts || []).map(a => String(a.id)));
  const unlinkedIncome = data.transactions.filter(t =>
    t.type === "income" &&
    !t.isGoalAccountTx &&
    (!t.bankId || (!linkedBankIds.has(String(t.bankId)) && !goalAccountIds.has(String(t.bankId))))
  ).reduce((s, t) => s + Number(t.amount || 0), 0);
  const unlinkedExpense = data.transactions.filter(t =>
    t.type === "expense" &&
    !t.isGoalAccountTx &&
    (!t.bankId || (!linkedBankIds.has(String(t.bankId)) && !goalAccountIds.has(String(t.bankId))))
  ).reduce((s, t) => s + Number(t.amount || 0), 0);
  ...
  const netWorth = ((data.banks || []).reduce((s, b) => {
    ...
    const exp = data.transactions.filter(t =>
      t.type === "expense" &&
      String(t.bankId) === String(b.id) &&
      !t.isGoalAccountTx
    ).reduce((a, t) => a + Number(t.amount || 0), 0);
...
function Overview({ data, netWorth, foNetPnl, setPage, toggles, update, portfolioOn }) {
  ...
  const filteredIncome  = txs.filter(t => t.type === "income"  && matchesPeriod(t.date) && !t.isGoalAccountTx).reduce((s, t) => s + Number(t.amount), 0);
  const filteredExpense = txs.filter(t => t.type === "expense" && matchesPeriod(t.date) && !t.isGoalAccountTx).reduce((s, t) => s + Number(t.amount), 0);
  ...
  // Bank balances: sum income - expense per bank (exclude credit cards and goal account transactions)
  const banks = data.banks || [];
  const bankBalances = banks.filter(bank => bank.type !== "Credit Card").map(bank => {
    const inc = data.transactions.filter(t =>
      t.type === "income" &&
      String(t.bankId) === String(bank.id) &&
      !t.isGoalAccountTx
    ).reduce((s, t) => s + Number(t.amount || 0), 0);
    const exp = data.transactions.filter(t =>
      t.type === "expense" &&
...
function BudgetTab({ data, update, categories }) {
  ...
  // Goals: exclude tasks, split by kind, sort by goalNumber
  const moneyNeeds = (data.needsWants || []).filter(g => g.goalType !== "task" && g.kind === "need").sort((a,b) => (a.goalNumber||0)-(b.goalNumber||0)).map(g => g.name).filter(Boolean);
  const moneyWants = (data.needsWants || []).filter(g => g.goalType !== "task" && g.kind === "want").sort((a,b) => (a.goalNumber||0)-(b.goalNumber||0)).map(g => g.name).filter(Boolean);
  const moneyGoals = moneyNeeds; // alias for optgroup rendering
...
function MoneyPage({ data, update, tab, setTab }) {
  ...
  const expense = data.transactions.filter(t => t.type === "expense" && filterByPeriod(t.date, period) && !t.isGoalAccountTx).reduce((s, t) => s + Number(t.amount), 0);
...
// ─── Settings ─────────────────────────────────────────────────────────────────
function SettingsPage({ data, update, tab, setTab, navItems, navEditMode, setNavEditMode, onNavDragStart, onNavDragOver, onNavDrop, navDragOver, navDragIdx, setNavDragOver, firebaseUser }) {
  ...
  return (
    <div>
      ...
      {/* ── Backup ── */}
      {effectiveTab === "backup" && <BackupSettings data={data} update={update} cardStyle={cardStyle} sectionTitle={sectionTitle} firebaseUser={firebaseUser} />}

      {/* ── Docs ── */}
      {effectiveTab === "docs" && <DocsPage data={data} update={update} />}
    </div>
  );
}
...
// ─── BackupSettings ────────────────────────────────────────────────────────────
function BackupSettings({ data, update, cardStyle, sectionTitle, firebaseUser }) {
  ...
  const gdrive = data.gdriveIntegration || { connected: false };
  const isConnected = !!gdrive.connected;
  const FOLDER_NAME   = "FinTracker";
  const BACKUP_FOLDER = "Backup";
  ...
  async function uploadBackup(token, backupFolderId, silent = false) {
    ...
    if (existingFileId) {
      ...
      uploadRes = await fetch(`https://www.googleapis.com/upload/drive/v3/files/${existingFileId}?uploadType=multipart&fields=id,name`,
        { method: "PATCH", headers: { Authorization: `Bearer ${token}`, "Content-Type": `multipart/related; boundary=${boundary}` }, body }
      );
    } else {
      const blob = new Blob([jsonStr], { type: "application/json" });
      const formData = new FormData();
      formData.append("metadata", new Blob([JSON.stringify({ name: FILE_NAME, parents: [backupFolderId] })], { type: "application/json" }));
      formData.append("file", blob);
      uploadRes = await fetch("https://www.googleapis.com/upload/drive/v3/files?uploadType=multipart&fields=id,name",
  ...
  useEffect(() => {
    ...
    const interval = setInterval(runAutoBackup, SIX_HOURS);
    return () => clearInterval(interval);
  }, [isConnected, lastAutoBackup, data]);
  ...
  function handleDisconnect() {
    ...
    setShowFiles(false);
    setDriveFiles([]);
    flashMsg("success", "✅ Disconnected from Google Drive.");
  }
  ...
  async function handleSaveToDrive() {
    ...
    try {
      ...
      const folderId = await ensureFinTrackerFolder();
      const backupFolderId = await ensureBackupFolder(folderId);
      const ok = await uploadBackup(token, backupFolderId, false);
      if (ok) {
        const now = Date.now();
        localStorage.setItem("ft_last_auto_backup", now.toString());
        setLastAutoBackup(now);
  ...
  async function handleListFiles() {
    ...
    try {
      const token = await getStoredToken();
      if (!token) { setLoadingFiles(false); return; }
      const folderId = await ensureFinTrackerFolder();
      const backupFolderId = await ensureBackupFolder(folderId);
      const res = await driveRequest(
        `https://www.googleapis.com/drive/v3/files?q='${backupFolderId}'+in+parents+and+trashed%3Dfalse&fields=files(id,name,size,createdTime)&orderBy=createdTime desc`
      );
      setDriveFiles(res.files || []);
      setShowFiles(true);
    } catch (e) { handleDriveError(e); }
  ...
  async function handleRestore(file) {
    ...
    try {
      ...
      flashMsg("success", `✅ Restored from Drive: ${file.name}`);
      setShowFiles(false);
    } catch (e) { flashMsg("error", `❌ Restore failed: ${e.message}`); }
    finally { setStatus(null); }
  }
      ...
      {accounts.length > 0 && (
        <div style={cardStyle}>
          ...
          <DraggableList
            ...
            renderItem={acct => {
              const txInc = data.transactions.filter(t => t.type === "income" && String(t.bankId) === String(acct.id) && !t.isGoalAccountTx).reduce((s, t) => s + Number(t.amount), 0);
              const txExp = data.transactions.filter(t => t.type === "expense" && String(t.bankId) === String(acct.id) && !t.isGoalAccountTx).reduce((s, t) => s + Number(t.amount), 0);
              let bal;
...
function BusinessPageInner({ data, update }) {
  ...
  async function uploadBill(entryId, file) {
    if (!activeBiz || !file) return;
    ...
    try {
      ...
      // Generate filename: bill_feb_1.pdf, bill_feb_2.pdf, etc.
      const monthName = entry.month.toLowerCase().substring(0, 3); // jan, feb, mar, etc.
      const fileExtension = file.name.split('.').pop();
      const fileName = billNumber === 1 
      ...
      updateBizData(d => d.map(e => 
        e.id === entryId ? { ...e, bills: [...(e.bills || []), newBill] } : e
      ));
  ...
  async function deleteBill(entryId, bill) {
    ...
    try {
      ...
      updateBizData(d => d.map(e => 
        e.id === entryId ? { ...e, bills: (e.bills || []).filter(b => b.id !== bill.id) } : e
      ));
      
      alert("Bill deleted successfully!");
    } catch (error) {
      console.error("Delete error:", error);
      alert("Failed to delete bill from Google Drive, but removed from app.");
      // Still remove from app even if Drive delete fails
      updateBizData(d => d.map(e => 
        e.id === entryId ? { ...e, bills: (e.bills || []).filter(b => b.id !== bill.id) } : e
      ));
    } finally {
                        ...
                        <div style={STYLES.s255}>
                          ...
                          <label style={{ cursor: uploadingBill ? "not-allowed" : "pointer" }}>
                            <input
                              ...
                              onChange={async (e) => {
                                const file = e.target.files?.[0];
                                if (file) await uploadBill(entry.id, file);
                                e.target.value = "";
                              }}
                            />
                            <span style={{
                              ...
                              gap: 6,
                              padding: "5px 12px",
                              background: uploadingBill ? "#ccc" : "#e8f5ee",
                              color: uploadingBill ? "#999" : "#1a6b3c",
                              border: "none",
                              borderRadius: 6,
                              fontSize: 12,
                              fontWeight: 500
                            ...
                            {(entry.bills || []).map((bill, idx) => (
                              ...
                              >
                                ...
                                <div style={STYLES.s441}>
                                  ...
                                  <a
                                    href={bill.downloadUrl || bill.url}
                                    download={bill.name}
                                    target="_blank"
                                    rel="noreferrer"
                                    style={STYLES.s848}
                                  ...
                                  <button
                                    onClick={() => deleteBill(entry.id, bill)}
                                    disabled={billToDelete === bill.id}
                                    style={{
                                      background: "#fee2e2",
                                      color: "#ef4444",
                                      border: "none",
                                      borderRadius: 4,
                                      padding: "4px 10px",
                                      cursor: billToDelete === bill.id ? "not-allowed" : "pointer",
                                      fontSize: 11,
                                      fontWeight: 600,
                                      opacity: billToDelete === bill.id ? 0.5 : 1
                                    }}
                                  >
...
function DocsPage({ data, update }) {
  // Navigation: null = root, folderId = inside folder, { folderId, subId } = inside subfolder
  const folders = data.docsFolders || [];

  // Level state
  const [selectedFolder,    setSelectedFolder]    = useState(null); // folder id
  const [selectedSub,       setSelectedSub]       = useState(null); // subfolder id

  // Add/rename state — folders
  const [showAddFolder,     setShowAddFolder]     = useState(false);
  const [newFolderName,     setNewFolderName]     = useState("");
  const [renamingFolder,    setRenamingFolder]    = useState(null); // { id, value }

  // Add/rename state — subfolders
  const [showAddSub,        setShowAddSub]        = useState(false);
  const [newSubName,        setNewSubName]        = useState("");
  const [renamingSub,       setRenamingSub]       = useState(null); // { id, value }
  ...
  const subfolders   = activeFolder ? (activeFolder.subfolders || []) : [];
  const activeSub    = subfolders.find(s => s.id === selectedSub) || null;
  const docs         = activeSub ? (activeSub.docs || []) : [];

  // ── helpers ──────────────────────────────────────────────────────────────
  function updateFolders(fn) {
    update(p => ({ docsFolders: fn(p.docsFolders || []) }));
  }
  function updateSubfolders(fn) {
    updateFolders(fs => fs.map(f => f.id === selectedFolder ? { ...f, subfolders: fn(f.subfolders || []) } : f));
  }
  function updateDocs(fn) {
    updateSubfolders(ss => ss.map(s => s.id === selectedSub ? { ...s, docs: fn(s.docs || []) } : s));
  }

  // ── folder CRUD ───────────────────────────────────────────────────────────
  function addFolder() {
    const name = newFolderName.trim(); if (!name) return;
    const f = { id: "docf_" + Date.now(), name, subfolders: [], createdAt: new Date().toISOString() };
    updateFolders(fs => [...fs, f]);
  ...
  function deleteFolder(id) {
    ...
    if (selectedFolder === id) { setSelectedFolder(null); setSelectedSub(null); }
  }
  function renameFolder(id, val) {
  ...
  function addSub() {
    ...
    const s = { id: "docs_" + Date.now(), name, docs: [], createdAt: new Date().toISOString() };
    updateSubfolders(ss => [...ss, s]);
    setNewSubName(""); setShowAddSub(false);
    setSelectedSub(s.id);
  }
  function deleteSub(id) {
    if (!confirm("Delete this subfolder and all its documents?")) return;
    updateSubfolders(ss => ss.filter(s => s.id !== id));
    if (selectedSub === id) setSelectedSub(null);
  }
  function renameSub(id, val) {
    const n = val.trim(); if (!n) return;
    updateSubfolders(ss => ss.map(s => s.id === id ? { ...s, name: n } : s));
    setRenamingSub(null);
  }

  // ── doc upload / delete ───────────────────────────────────────────────────
  async function uploadDoc(file) {
    if (!activeFolder || !activeSub || !file) return;
    setUploadingDoc(true);
    try {
      const token = await getFreshDriveToken();
      if (!token) { alert("Please sign in with Google to upload documents."); return; }
      // Drive path: FinTracker/Docs/<Folder>/<Subfolder>/
      const finTrackerId  = await ensureDriveFolder(token, "FinTracker");
      const docsRootId    = await ensureDriveFolder(token, "Docs",           finTrackerId);
      const folderDriveId = await ensureDriveFolder(token, activeFolder.name, docsRootId);
      const subDriveId    = await ensureDriveFolder(token, activeSub.name,   folderDriveId);
      const count         = docs.length + 1;
      const ext           = file.name.split(".").pop();
      const base          = file.name.replace(/\.[^/.]+$/, "");
      const fileName      = count === 1 ? file.name : `${base}_${count}.${ext}`;
      const driveFile     = await uploadFileToDrive(token, file, fileName, subDriveId);
      const newDoc = { id: driveFile.id, name: fileName, originalName: file.name, url: driveFile.webViewLink, downloadUrl: driveFile.webContentLink, size: driveFile.size, mimeType: driveFile.mimeType, uploadedAt: new Date().toISOString() };
  ...
  async function deleteDoc(doc) {
    ...
    try {
      ...
      updateDocs(ds => ds.filter(d => d.id !== doc.id));
    } catch (err) {
  ...
  function getFileIcon(mimeType, name) {
    ...
    if (m.includes("zip")          || /\.(zip|rar|7z)$/.test(n))                  return "🗜️";
  ...
  // ── grid card (shared for folders + subfolders) ───────────────────────────
  ...
  // ── total doc count helper ────────────────────────────────────────────────
  function folderDocCount(folder) {
    return (folder.subfolders || []).reduce((s, sub) => s + (sub.docs || []).length, 0);
  }
    ...
    <div style={STYLES.s910}>
      ...
      {!selectedFolder && (
        <>
          ...
          <div style={cardStyle}>
            {showAddFolder && (
              <AddRow value={newFolderName} onChange={setNewFolderName} onConfirm={addFolder}
              ...
              <div style={STYLES.s914}>
                {folders.map(folder => (
                  <GridCard key={folder.id}
                    ...
                    sub={`${(folder.subfolders || []).length} subfolder${(folder.subfolders || []).length !== 1 ? "s" : ""} · ${folderDocCount(folder)} doc${folderDocCount(folder) !== 1 ? "s" : ""}`}
                    isRenaming={renamingFolder?.id === folder.id} renamingVal={renamingFolder?.value || ""}
                    onRenameChange={e => setRenamingFolder(r => ({ ...r, value: e.target.value }))}
                    onRenameKey={e => { e.stopPropagation(); if (e.key === "Enter") renameFolder(folder.id, renamingFolder.value); if (e.key === "Escape") setRenamingFolder(null); }}
                    onRenameBlur={() => renameFolder(folder.id, renamingFolder?.value || "")}
                    onClick={() => setSelectedFolder(folder.id)}
                    onRename={() => setRenamingFolder({ id: folder.id, value: folder.name })}
      ...
      {/* ══ LEVEL 1: Inside a folder — subfolder grid ════════════════════════ */}
      {selectedFolder && !selectedSub && activeFolder && (
        <>
          ...
          <div style={cardStyle}>
            {showAddSub && (
              <AddRow value={newSubName} onChange={setNewSubName} onConfirm={addSub}
            ...
            {subfolders.length === 0 ? (
              ...
              <div style={STYLES.s914}>
                {subfolders.map(sub => (
                  <GridCard key={sub.id}
                    emoji="📂" name={sub.name}
                    sub={`${subDocCount(sub)} doc${subDocCount(sub) !== 1 ? "s" : ""}`}
                    isRenaming={renamingSub?.id === sub.id} renamingVal={renamingSub?.value || ""}
                    onRenameChange={e => setRenamingSub(r => ({ ...r, value: e.target.value }))}
                    onRenameKey={e => { e.stopPropagation(); if (e.key === "Enter") renameSub(sub.id, renamingSub.value); if (e.key === "Escape") setRenamingSub(null); }}
                    onRenameBlur={() => renameSub(sub.id, renamingSub?.value || "")}
                    onClick={() => setSelectedSub(sub.id)}
                    onRename={() => setRenamingSub({ id: sub.id, value: sub.name })}
      ...
      {/* ══ LEVEL 2: Inside a subfolder — document list ══════════════════════ */}
      {selectedFolder && selectedSub && activeSub && (
        <>
          <Breadcrumb />
          <div style={cardStyle}>
            <div style={STYLES.s139}>
                ...
                {docs.map(doc => (
                  <div key={doc.id} style={STYLES.s916}>
                    ...
                    <div style={STYLES.s441}>
                      ...
                      <button onClick={() => deleteDoc(doc)} disabled={deletingDoc === doc.id}
