...
function BusinessPage({ data, update }) {
  ...
  return (
    <div>
        ...
        <>
          ...
          {yearEntries.length === 0 ? (
            ...
            <>
              ...
              {/* ── Non-day-wise Month Detail Panel (shown inline, replacing grid) ── */}
              {selectedNonDayMonth && !isDayWise && (() => {
                  ...
                  return (
                    <div style={{ background: "var(--color-background-primary)", borderRadius: 14, border: "0.5px solid var(--color-border-secondary)", padding: "1.2rem 1.4rem", marginBottom: 16 }}>
                      ...
                      <div style={{ marginBottom: 16 }}>
                        ...
                        {editing ? (
                          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr auto", gap: 6, marginBottom: 2 }}>
                              <div style={{ fontSize: 11, fontWeight: 600, color: "var(--color-text-secondary)", padding: "0 2px" }}>Qty (units)</div>
                              <div style={{ fontSize: 11, fontWeight: 600, color: "var(--color-text-secondary)", padding: "0 2px" }}>Price (₹)</div>
                              <div style={{ fontSize: 11, fontWeight: 600, color: "var(--color-text-secondary)", padding: "0 2px", textAlign: "right" }}>Subtotal</div>
                              <div style={{ width: 28 }} />
                            </div>
                            {editRows.map((row, idx) => {
                              ...
                              return (
                                <div key={idx} style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr auto", gap: 6, alignItems: "center" }}>
                                  <input type="text" inputMode="decimal" placeholder="e.g. 10"
                                    value={row.qty}
                ...
                return <DetailPanelInline key={entry.id} />;
              })()}
      ...
      {/* ── Non-day-wise Month Detail Panel (moved inline above) ── */}
      {false && (() => {
        const entry = bizData.find(e => e.id === selectedNonDayMonth);
        if (!entry) return null;
        const gross = entry.grossIncome || 0;
